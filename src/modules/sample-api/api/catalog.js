 ...
 *
 * Details: https://sfa-docs.now.sh/guide/default-modules/api.html#vsbridgecatalog
 */
export default ({ config, db }) =>
  function (req, res, body) {
    ...
    if (urlSegments.length < 2) {
      throw new Error(
        'No index name given in the URL. Please do use following URL format: /api/catalog/<index_name>/<entity_type>_search'
      );
    } else {
      ...
      if (config.elasticsearch.indices.indexOf(indexName) < 0) {
        throw new Error(
          'Invalid / inaccessible index name given in the URL. Please do use following URL format: /api/catalog/<index_name>/_search'
        );
      }

      if (urlSegments[urlSegments.length - 1].indexOf('_search') !== 0) {
        throw new Error(
          'Please do use following URL format: /api/catalog/<index_name>/<entity_name>/_search'
        );
      }
    }
    ...
    let url =
      ...
      (req.query.request
        ? _updateQueryStringParameter(req.url, 'request', null)
        : req.url);
    ...
    request(
      ...
      (_err, _res, _resBody) => {
        // TODO: add caching layer to speed up SSR? How to invalidate products (checksum on the response BEFORE processing it)
        res.json(_resBody);
      }
    );
  };
